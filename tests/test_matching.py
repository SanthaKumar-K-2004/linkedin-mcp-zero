...
async def test_match_jobs_to_resume(mock_dependencies) -> None:
    ...
    # Save a resume
    resume_id = storage.save_resume("/dummy/path", {
        "name": "Jane",
        "skills": ["python", "docker"],
    })
    
    # Mock search jobs returning job postings
    public.search_jobs = AsyncMock(return_value=[
        {"id": "1", "t": "Python Dev", "co": "Google", "loc": "NY", "url": "url1"}
    ])
    public.get_job_details = AsyncMock(return_value={
        "id": "1",
        "t": "Python Dev",
...
async def test_compare_jobs(mock_dependencies) -> None:
    ...
    public.get_job_details = AsyncMock(side_effect=[
...
async def test_export_jobs_json(mock_dependencies, tmp_path) -> None:
    ...
    public.get_job_details = AsyncMock(return_value={
        "co": "Google", "t": "Python Dev", "sal": "$150K", "loc": "NY"
    })
...
async def test_export_jobs_csv(mock_dependencies, tmp_path) -> None:
    ...
    public.get_job_details = AsyncMock(return_value={
        "co": "Google", "t": "Python Dev", "sal": "$150K", "loc": "NY", "id": "1", "url": "url1", "type": "FT"
    })
